/*!
 * @file moto_SSD1306.cpp
 *
 * @mainpage Arduino library for monochrome OLEDs based on SSD1306 drivers.
 *
 * @section intro_sec Introduction
 *
 * This is documentation for moto's SSD1306 library for monochrome
 * OLED displays: http://www.moto.com/category/63_98
 *
 ...
 *
 * This library depends on <a href="https://github.com/moto/moto-GFX-Library">
 * moto_GFX</a> being present on your system. Please make sure you have
 * installed the latest version before using this library.
 *
 * @section author Author
 *
 * Written by Limor Fried/Ladyada for moto Industries, with
...
/*!
    ...
    @return moto_SSD1306 object.
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
moto_SSD1306::moto_SSD1306(uint8_t w, uint8_t h, TwoWire *twi,
  int8_t rst_pin, uint32_t clkDuring, uint32_t clkAfter) :
  moto_GFX(w, h), spi(NULL), wire(twi ? twi : &Wire), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
moto_SSD1306::moto_SSD1306(uint8_t w, uint8_t h,
  int8_t mosi_pin, int8_t sclk_pin, int8_t dc_pin, int8_t rst_pin,
  int8_t cs_pin) : moto_GFX(w, h), spi(NULL), wire(NULL), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
moto_SSD1306::moto_SSD1306(uint8_t w, uint8_t h, SPIClass *spi,
  int8_t dc_pin, int8_t rst_pin, int8_t cs_pin, uint32_t bitrate) :
  moto_GFX(w, h), spi(spi ? spi : &SPI), wire(NULL), buffer(NULL),
...
/*!
    @brief  DEPRECATED constructor for SPI SSD1306 displays, using software
            ...
            one of the SSD1306_* size defines in moto_SSD1306.h. New
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
moto_SSD1306::moto_SSD1306(int8_t mosi_pin, int8_t sclk_pin,
  int8_t dc_pin, int8_t rst_pin, int8_t cs_pin) :
  moto_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
moto_SSD1306::moto_SSD1306(int8_t dc_pin, int8_t rst_pin,
  int8_t cs_pin) : moto_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT),
...
/*!
    @brief  DEPRECATED constructor for I2C SSD1306 displays. Provided for
            ...
            defines in moto_SSD1306.h. New code should NOT use this.
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
moto_SSD1306::moto_SSD1306(int8_t rst_pin) :
  moto_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(&Wire),
  buffer(NULL), mosiPin(-1), clkPin(-1), dcPin(-1), csPin(-1),
  rstPin(rst_pin) {
}

/*!
    @brief  Destructor for moto_SSD1306 object.
*/
moto_SSD1306::~moto_SSD1306(void) {
  if(buffer) {
    free(buffer);
    buffer = NULL;
  }
...
inline void moto_SSD1306::SPIwrite(uint8_t d) {
  if(spi) {
    (void)spi->transfer(d);
  } else {
...
void moto_SSD1306::ssd1306_command1(uint8_t c) {
  if(wire) { // I2C
...
void moto_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if(wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint8_t bytesOut = 1;
...
/*!
    ...
    @return None (void).
*/
void moto_SSD1306::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  ssd1306_command1(c);
  TRANSACTION_END
}
...
/*!
    ...
    @param  vcs
            ...
            otherwise. Most situations with moto SSD1306 breakouts will
...
*/
boolean moto_SSD1306::begin(uint8_t vcs, uint8_t addr, boolean reset,
  boolean periphBegin) {
...
/*!
    @brief  Set/clear/invert a single pixel. This is also invoked by the
            moto_GFX library in generating many higher-level graphics
            primitives.
    @param  x
...
*/
void moto_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void moto_SSD1306::clearDisplay(void) {
  memset(buffer, 0, WIDTH * ((HEIGHT + 7) / 8));
}

/*!
    @brief  Draw a horizontal line. This is also invoked by the moto_GFX
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void moto_SSD1306::drawFastHLine(
...
void moto_SSD1306::drawFastHLineInternal(
  int16_t x, int16_t y, int16_t w, uint16_t color) {
...
/*!
    @brief  Draw a vertical line. This is also invoked by the moto_GFX
            library in generating many higher-level graphics primitives.
    @param  x
            Column of display -- 0 at left to (screen width -1) at right.
    @param  y
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void moto_SSD1306::drawFastVLine(
...
void moto_SSD1306::drawFastVLineInternal(
...
*/
boolean moto_SSD1306::getPixel(int16_t x, int16_t y) {
  if((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch(getRotation()) {
     case 1:
...
*/
uint8_t *moto_SSD1306::getBuffer(void) {
  return buffer;
}
...
*/
void moto_SSD1306::display(void) {
  TRANSACTION_START
...
void moto_SSD1306::startscrollright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void moto_SSD1306::startscrollleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void moto_SSD1306::startscrolldiagright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void moto_SSD1306::startscrolldiagleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void moto_SSD1306::stopscroll(void) {
  TRANSACTION_START
...
*/
void moto_SSD1306::invertDisplay(boolean i) {
  TRANSACTION_START
...
*/
void moto_SSD1306::dim(boolean dim) {
  uint8_t contrast;
